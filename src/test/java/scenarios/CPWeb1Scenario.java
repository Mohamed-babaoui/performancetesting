...
public class CPWeb1Scenario {
    ...
    public CPWeb1Scenario() {
        config = new Properties();
        try {
            config.load(CPWeb1Scenario.class.getClassLoader().getResourceAsStream("config.properties"));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        testData = new Properties();
        try {
            testData.load(CPWeb1Scenario.class.getClassLoader().getResourceAsStream("testdata2.properties"));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }

        //baseUrl = System.getProperty("url");
        baseUrl="https://bookparcs-a5.pvcp.intra/polo-booking/booking/view/login/screen/login.jsf";
                ...
                .on(
                    ...
                    WebDriver driver = BrowserManager.getWebDriver(driver_id);*/
                                ...
                                .doIf(session -> !session.isFailed()).then(exec(session -> {
                                    ...
                                    if (!customerSearchPage.searchCustomerByNameData("HAMDI", "JAMEL", "75019"))
